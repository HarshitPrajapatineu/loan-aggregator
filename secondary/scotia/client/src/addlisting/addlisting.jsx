...
//   const saveData = () => {
//     let url = `http://localhost:8082/scotia/getUsers`;
//     axios
//       .get(url)
//       .then((res) => {
//         console.log("dataa");
...
function AddListing() {
  ...
  const saveData = () => {
    let url = "http://localhost:8082/scotia/" + (refid ? "updateListing" : "addListing");
    let data = {
      "id": 0,
  ...
  const getData = () => {
    let url = `http://localhost:8082/scotia/getListing/` + refid;
    axios
      .get(url)
      .then((res) => {
        console.log(res.data);
