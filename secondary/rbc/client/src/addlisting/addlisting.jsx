...
//   const saveData = () => {
//     let url = `http://localhost:8085/rbc/getUsers`;
//     axios
//       .get(url)
//       .then((res) => {
//         console.log("dataa");
...
function AddListing() {
  ...
  const saveData = () => {
    let url = "http://localhost:8085/rbc/" + (refid ? "updateListing" : "addListing");
    let data = {
      "id": 0,
  ...
  const getData = () => {
    let url = `http://localhost:8085/rbc/getListing/` + refid;
    axios
      .get(url)
      .then((res) => {
        console.log(res.data);
