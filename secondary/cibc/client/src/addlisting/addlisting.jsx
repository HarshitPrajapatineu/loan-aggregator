...
//   const saveData = () => {
//     let url = `http://localhost:8084/cibc/getUsers`;
//     axios
//       .get(url)
//       .then((res) => {
//         console.log("dataa");
...
function AddListing() {
  ...
  const saveData = () => {
    let url = "http://localhost:8084/cibc/" + (refid ? "updateListing" : "addListing");
    let data = {
      "id": 0,
  ...
  const getData = () => {
    let url = `http://localhost:8084/cibc/getListing/` + refid;
    axios
      .get(url)
      .then((res) => {
        console.log(res.data);
